 ...
 * @property {Record<string, boolean|string>} [globals] The global variable settings.
...
class ConfigArrayFactory {
    ...
    _normalizeConfigData(configData, filePath, name) {
        ...
        if (!name) {
            name = filePath && path.relative(cwd, filePath);
        }
    ...
    *_normalizeObjectConfigDataBody(
    ...
    _loadParser(nameOrPath, importerPath, importerName) {
        ...
        const { additionalParserPool, cwd } = internalSlotsMap.get(this);
        const relativeTo = importerPath || path.join(cwd, ".eslintrc");

        // Check for the additional pool.
        if (!isFilePath(nameOrPath) && additionalParserPool.has(nameOrPath)) {
            return new ConfigDependency({
                definition: additionalParserPool.get(nameOrPath),
